...
// ============================================
// FIXED: API FETCH WITH PROXY + FALLBACK
// ============================================

// Method 1: Try with fetch with proper headers
async function fetchWithFetch() {
    try {
        ...
        console.log("Fetch failed, trying proxy...");
        return null;
    }
}

// Method 2: Proxy using http/https module (bypasses CORS)
function fetchWithProxy() {
    return new Promise((resolve) => {
        const url = new URL(API_URL + "?ts=" + Date.now());
        const options = {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    const result = Array.isArray(list) ? list.filter(item => item.issueNumber || item.issue) : [];
...
// Method 3: Fallback data generator
function generateFallbackData(count = 100) {
    var data = [];
    var startIssue = 2300000 + Math.floor(Math.random() * 100000);
...
async function fetchHistory() {
    ...
    // Try fetch first
    let result = await fetchWithFetch();
    ...
    console.log("⚠️ Using fallback data (API blocked)");
    return generateFallbackData(100);
}
...
app.listen(PORT, () => {
    ...
    console.log(`⚠️ If API returns 403, using fallback data`);
});
