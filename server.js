...
// ============================================
// COMPLETE TRIDENT V23 AI ENGINE
// ============================================
...
// ============================================
// LEVEL 2 — DEEP PATTERN AI ENGINE
// ============================================

// Multi-Length Pattern Match
...
// ============================================
// FETCH API DATA WITH PROPER PERIOD HANDLING
// ============================================
async function fetchHistory() {
    ...
    try {
        const response = await fetch(API_URL + "?ts=" + now, {
            method: "GET",
            headers: { 
        ...
        // Filter valid records and sort by issueNumber descending
        const validRecords = list
            .filter(item => {
                const num = item.number || item.num;
                const issue = item.issueNumber || item.issue;
                return num !== undefined && num !== null && issue;
            })
            .sort((a, b) => {
                const issueA = parseInt(a.issueNumber || a.issue || "0");
                const issueB = parseInt(b.issueNumber || b.issue || "0");
                return issueB - issueA; // Descending order - latest first
            });
...
// Fallback data
function generateFallbackData(count = 100) {
    var data = [];
    var startIssue = 24062000001;
    for (var i = 0; i < count; i++) {
        data.push({
            number: String(Math.floor(Math.random() * 10)),
            issueNumber: String(startIssue + i),
...
app.get('/api/verify-key', verifyLimiter, (req, res) => {
    ...
    console.log(`Key verification: ${isValid ? 'SUCCESS' : 'FAILED'} - IP: ${req.ip}`);
    res.json({ valid: isValid, ...(isValid && { timestamp: new Date().toISOString() }) });
});
...
app.get('/api/predict', async (req, res) => {
    ...
    const trimmedKey = userKey.trim();
    if (trimmedKey !== SECRET_KEY) {
        console.log(`Prediction denied - Invalid key from IP: ${req.ip}`);
        return res.status(401).json({ error: "Invalid Key", code: "AUTH_FAILED" });
    }
    ...
    if (!records || records.length === 0) {
        return res.status(503).json({ error: "Unable to fetch data from API" });
    }

    // Get latest result (first record after sorting)
    const latest = records[0];
    
    // Get latest issue number from API
    let latestIssueNumber = 0;
    const rawIssue = latest.issueNumber || latest.issue || "0";
    
    // Parse issue number - handle different formats
    const issueMatch = String(rawIssue).match(/(\d+)/);
    if (issueMatch) {
        latestIssueNumber = parseInt(issueMatch[1]);
    }
    
    const latestNumber = parseInt(latest.number || latest.num || 0);
    const latestCat = latestNumber >= 5 ? "BIG" : "SMALL";

    // Generate prediction using AI engine
    const result = getPrediction(records);
    
    // Calculate next period = latest issue + 1
    const nextPeriod = latestIssueNumber > 0 ? String(latestIssueNumber + 1) : "------";

    const response = {
        prediction: result.prediction,
        confidence: fmt(result.confidence),
        timestamp: new Date().toISOString(),
        engine: "PS PVT MOD V23 | ADVANCED AI 2-LEVEL",
        status: result.status,
        latestResult: {
            issue: String(rawIssue),
            number: latestNumber,
            category: latestCat
        },
    ...
    console.log(`Prediction: ${result.prediction} for Period ${nextPeriod} (Last: ${rawIssue}) - IP: ${req.ip}`);
    res.json(response);
});
...
app.get('/api/latest-result', async (req, res) => {
    ...
    const records = await fetchHistory();
    if (!records || records.length === 0) {
        return res.status(503).json({ error: "Unable to fetch data" });
    }
    ...
    const issueMatch = String(rawIssue).match(/(\d+)/);
    const latestIssueNumber = issueMatch ? parseInt(issueMatch[1]) : 0;
    const nextPeriod = latestIssueNumber > 0 ? String(latestIssueNumber + 1) : "------";
...
app.listen(PORT, () => {
    console.log('========================================');
    console.log(`  PS PVT MOD Server V23 FIXED`);
    console.log(`  Port: ${PORT}`);
    console.log(`  Key: ${SECRET_KEY}`);
    console.log(`  API: ${API_URL}`);
    console.log(`  Period: API Last Issue + 1`);
    console.log('========================================');
});
