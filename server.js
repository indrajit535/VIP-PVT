...
// ============================================
const SECRET_KEY = "PS263"; // Default login password
...
// ============================================
// LEVEL 1 — STATISTICAL AI ENGINE
// ============================================

// 1. Multi-Decay Bayesian
...
// 6. Weighted Moving Average Crossover
...
// ============================================
// FETCH API DATA (Using Native fetch - Node 18+)
// ============================================
async function fetchHistory() {
    try {
        ...
        return Array.isArray(list) ? list.filter(item => item.issueNumber || item.issue) : [];
    } catch (error) {
...
app.get('/api/predict', async (req, res) => {
    ...
    // Calculate next period (current period + 1)
    let nextPeriod = "------";
    try {
        const currentNum = parseInt(latestIssue);
        if (!isNaN(currentNum)) {
            nextPeriod = String(currentNum + 1);
        }
    } catch(e) {
        nextPeriod = "------";
    }
    ...
    console.log(`✅ Prediction: ${result.prediction} (${fmt(result.confidence)}) - IP: ${req.ip}`);
    res.json(response);
});
...
app.get('/api/latest-result', async (req, res) => {
    ...
    const latest = records[0];
    res.json({
        issue: latest.issueNumber || latest.issue,
        number: parseInt(latest.number || 0),
        category: parseInt(latest.number || 0) >= 5 ? "BIG" : "SMALL"
    });
});

app.get('/api/health', (req, res) => {
    res.json({ status: "online", timestamp: new Date().toISOString(), version: "PS PVT MOD V23" });
});
...
app.listen(PORT, () => {
    ...
    console.log(`🔒 SECRET_KEY: ${SECRET_KEY} (default login: PS263)`);
    console.log(`🧠 AI Engine: L1 (STAT AI) + L2 (DEEP AI) combined`);
    console.log(`🛡️ Rate limiting: 100 requests per 15 minutes`);
    console.log(`📡 API URL: ${API_URL}`);
    console.log(`ℹ️  Using Node ${process.version}`);
});
